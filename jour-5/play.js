...
function startPlay() {
    ...
    prompt.get(["What is your number?"], function (err, res) {
        ...
        var playerNum = res;
        // console.log(playerNum['What is your number?']);

        // if(isNaN(playerNum) == "n" || isNaN(playerNum) == 'N'){
        //     throw new Error("Quiter"); //*********** */
        // } else
        if (isNaN(playerNum['What is your number?'])) {  // check this object value is num or symbole
            console.log("Error1", playerNum['What is your number?']);           // if symbole entered exit with error message
        } else {
    ...
    function play(numStart, numEnd) {
        ...
        if (counter <= 2) {     // give three changes to play if he loose
            startPlay();
        }
    }
